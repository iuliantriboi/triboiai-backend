...
(function() {
  ...
  function getDeviceFingerprint() {
    // Generează un ID unic pentru device bazat pe multiple caracteristici
    ...
    // Generează hash simplu din toate datele
    ...
    modal.innerHTML = `
      <style>
        ...
        #triboi-license-modal .modal-content {
          ...
          max-width: 480px;
          width: 90%;
          box-shadow: 0 20px 60px rgba(45, 212, 191, 0.4);
        }
        ...
        #triboi-license-modal h2 img {
          height: 48px;
          width: auto;
          filter: drop-shadow(0 0 12px rgba(45, 212, 191, 0.6)) 
                  drop-shadow(0 0 24px rgba(45, 212, 191, 0.4));
          transition: filter 0.3s ease;
        }

        #triboi-license-modal h2 img:hover {
          filter: drop-shadow(0 0 16px rgba(45, 212, 191, 0.8)) 
                  drop-shadow(0 0 32px rgba(45, 212, 191, 0.5));
        }

        #triboi-license-modal .subtitle {
          color: #94a3b8;
          margin: 0 0 25px 0;
          font-size: 14px;
        }
        ...
        #triboi-license-modal button:hover {
          transform: translateY(-2px);
          box-shadow: 0 10px 30px rgba(45, 212, 191, 0.5);
        }

        #triboi-license-modal .message {
          margin-top: 16px;
          padding: 12px;
          border-radius: 8px;
          font-size: 14px;
        ...
        #triboi-license-modal .message.error {
          background: rgba(239, 68, 68, 0.1);
          border: 1px solid rgba(239, 68, 68, 0.3);
          color: #ef4444;
        }

        #triboi-license-modal .message.success {
          background: rgba(45, 212, 191, 0.1);
          border: 1px solid rgba(45, 212, 191, 0.3);
          color: #2dd4bf;
        }

        #triboi-license-modal .info {
        ...
        #triboi-license-modal .info-title {
          color: #2dd4bf;
          font-weight: 700;
          margin-bottom: 10px;
        }

        #triboi-license-modal .info-item {
          color: #cbd5e1;
          margin: 6px 0;
        }

        #triboi-license-modal .info-item strong {
          color: #e5e7eb;
        }
      </style>
  ...
  function showExpiredModal(status) {
    ...
    modal.innerHTML = `
      <style>
        ...
        #triboi-expired-modal h2 {
          color: #ef4444;
          margin: 0 0 16px 0;
          font-size: 28px;
        }

        #triboi-expired-modal p {
          color: #cbd5e1;
          font-size: 15px;
          line-height: 1.6;
          margin-bottom: 24px;
        ...
        #triboi-expired-modal button:hover {
          transform: translateY(-2px);
        ...
        #triboi-expired-modal .close-btn:hover {
          background: #334155;
          box-shadow: none;
        }
      </style>
      ...
      <div class="modal-content">
        ...
        <div style="margin: 24px 0; padding: 20px; background: rgba(45, 212, 191, 0.05); border: 1px solid rgba(45, 212, 191, 0.2); border-radius: 12px;">
          <div style="color: #2dd4bf; font-weight: 600; margin-bottom: 12px; font-size: 14px;">
            🔑 Ai un cod nou?
          </div>
          <input 
            type="text" 
        ...
        <button class="close-btn" id="close-expired-btn">
          Închide
        </button>
      </div>
    `;
    ...
    renewBtn.onclick = function() {
      ...
      if (result.success) {
        ...
        setTimeout(() => {
          ...
          unlockWidget();
          updateStatusBar();
          location.reload(); // Refresh pentru a reseta totul
        }, 1200);
      } else {
    ...
    function showRenewMessage(text, type) {
      ...
      if (type === 'error') {
        renewMsg.style.background = 'rgba(239, 68, 68, 0.1)';
        renewMsg.style.border = '1px solid rgba(239, 68, 68, 0.3)';
        renewMsg.style.color = '#ef4444';
      } else {
        renewMsg.style.background = 'rgba(45, 212, 191, 0.1)';
        renewMsg.style.border = '1px solid rgba(45, 212, 191, 0.3)';
  ...
  function updateStatusBar() {
    ...
    if (status.valid) {
      ...
      // Get current language from index.html (default to 'ro')
      const currentLang = typeof window.currentLang !== 'undefined' ? window.currentLang : 'ro';
      const questionsText = typeof window.translations !== 'undefined' && window.translations[currentLang] ? 
                            window.translations[currentLang].questions : 'întrebări';
      const daysText = typeof window.translations !== 'undefined' && window.translations[currentLang] ? 
                       window.translations[currentLang].days : 'zile';
  ...
  function init() {
    ...
    if (!status.valid) {
      ...
      unlockWidget();
      updateStatusBar();
      console.log(`✓ Valid ${status.license.type}: ${status.questionsRemaining}Q, ${status.daysRemaining}D`);
    }
